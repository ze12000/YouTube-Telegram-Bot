...
async def start(_: Client , message: Message):
    await message.reply(
        '↯︙اهلاً بك عزيزي ↯︙في بوت التحميل من اليوتيوب\n\n- استخدم /help  لاظهار الاوامر',
...
async def _help(_: Client , message: Message):
    await message.reply(
        '1. /search استخدم الامر للبحث في اليوتيوب عن حاجه معينه\n\n2. /get لجلب معلومات فديو معين + الرابط بتاعه.\n\n3. /download لتحميل فديو من اليوتيوب.',
    )
...
async def search(_: Client , message: Message):
    ...
    try: ask = await listener.listen(
             ...
             text = '- ارسل الان الكلمه التي تريد تبحث عنها!\n\n- استخدم /cancel لنهاء العمليه التي بدتها.',
             reply_markup = ForceReply(selective = True, placeholder = 'ارسل الكلمه: '),
             reply_to_message_id = message.id,
             timeout = 30
         )
    ...
    markup = Markup([
        [
            Button('- اضعظ للتحميل -', callback_data = f'download {firstone["id"]} {ask.id}'),
        ],
        [
            Button('- اول بحث -', callback_data = 'nothing'),
            Button('- التالي -', callback_data = callback_data)
        ],
        [
            Button('- اليوتيوب -', url = yturl)
        ]
    ])
