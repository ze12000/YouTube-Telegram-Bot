...
async def download(_: Client , message: Message):
    ...
    try: type = await listener.listen(
             from_id = user_id,
             chat_id = message.chat.id,
             text = '- اختار يقلبي:\n- v اكتب حرف لتحميل الفديو.\n- a واكتب حرف لتحميل مقطع صوتي.\n\n- استخدم /cancel لانهاء العمليه.',
...
async def _next(_: Client , callback: CallbackQuery):
    ...
    markup = Markup([
        [
            Button('- اضغظ للتحميل -', callback_data = f'download {choosed["id"]} {callback.message.reply_markup.inline_keyboard[0][0].callback_data.split()[-1]}'),
        ],
        [
            Button('- السابق -' if int(index) - 1 != 0 else '- اول بحث -', callback_data = f'pre {int(index) - 1} {keyword}' if int(index) - 1 != 0 else 'nothing'),
            Button('- التالي -', callback_data = callback_data)
        ],
        [
            Button('- YouTube -', url = yturl)
...
async def downloadURL(message ,url, _type):
    ...
    await wait_msg.delete()
    wait_msg = await message.reply('- جاري الرفع....', reply_to_message_id = info_msg.id)
    ...
    if obj.isSuccessful():
        ...
        await info_msg.edit_media(media = func(media = path ,caption = icaption.replace('```ملاحظه\nيتم التحميل...```', f'\n\n- تم التحميل بواسطه: @{app.me.username}')))
